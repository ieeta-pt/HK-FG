...
foreach(t = 1:REP) %dopar% {
  ...
  for (i in 1:length(NN)){
    ...
    for(j in 1:num_iter){
      ...
      # Generate ARMA(p,q) values
      arma_values <- arima.sim(list(order = c(p,d,q), ar=c(phi1), ma=c(theta1)), n = n)
  ...
  file_name <- paste("try", t, "_rights_arma11_",phi1,"_",theta1,".txt", sep = "")
  lapply(rights, write, file_name, append = TRUE, ncolumns = length(NN))
  
  file_name <- paste("try", t, "_wrongs00_arma11_",phi1,"_",theta1,".txt", sep = "")
  lapply(wrongs_00, write, file_name, append = TRUE, ncolumns = length(NN))
  
  file_name <- paste("try", t, "_wrongs01_arma11_",phi1,"_",theta1,".txt", sep = "")
  lapply(wrongs_01, write, file_name, append = TRUE, ncolumns = length(NN))
  
  file_name <- paste("try", t, "_wrongs22_arma11_",phi1,"_",theta1,".txt", sep = "")
