...
foreach(t = 1:REP) %dopar% {
  ...
  for (i in 1:length(NN)){
    ...
    for(j in 1:num_iter){
      ...
      # Generate ARMA(0,q) values
      arma_values <- arima.sim(list(order = c(p,d,q), ma=c(theta1)), n = n)
  ...
  file_name <- paste("try", t, "_rights_ma1_",theta1,".txt", sep = "")
  lapply(rights, write, file_name, append = TRUE, ncolumns = length(NN))
  
  file_name <- paste("try", t, "_wrongs00_ma1_",theta1,".txt", sep = "")
  lapply(wrongs_00, write, file_name, append = TRUE, ncolumns = length(NN))
  
  file_name <- paste("try", t, "_wrongs10_ma1_",theta1,".txt", sep = "")
  lapply(wrongs_10, write, file_name, append = TRUE, ncolumns = length(NN))
  
  file_name <- paste("try", t, "_wrongs22_ma1_",theta1,".txt", sep = "")
